...
void CAPTURE::capture(CAPTURE_RESULT &result)
{
    ...
    auto isHotspotAddress = [](u_long address) -> bool
    {
        const u_long hostAddress = ntohl(address);
        return hostAddress == ((192u << 24) | (168u << 16) | (137u << 8) | 1u) ||
               ((hostAddress >> 8) == ((192u << 16) | (168u << 8) | 137u));
    };
    ...
    for (pcap_if_t *device = devices; device && !selectedDevice; device = device->next)
    {
        ...
        for (pcap_addr_t *addr = device->addresses; addr && !selectedDevice; addr = addr->next)
        {
            if (addr->addr && addr->addr->sa_family == AF_INET &&
                isHotspotAddress(((struct sockaddr_in *)addr->addr)->sin_addr.s_addr))
            {
